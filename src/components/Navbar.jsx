...
function Navbar() {
  ...
  useEffect(() => {
    if (isScrolled) {
      controls.start({ opacity: 0, transition: { delay: 0.2 } });
    } else {
      controls.start({ opacity: 1, transition: { delay: 0.2 } });
    }
  }, [isScrolled, controls]);
  ...
  return (
    <motion.div
      initial={{ opacity: 1 }}
      animate={controls}
      className='fixed z-[999] w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-6 font-["NeueMontreal-regular"] flex justify-between items-center'
    >
      <motion.div
        className='logo'
        initial={{ opacity: 1 }}
      ...
      <div className='links flex gap-4 sm:gap-6 lg:gap-8'>
        {['Services', 'Our work', 'About us', 'Insights', 'Contact us'].map(
          (item, index) => (
            <a
              key={index}
              className={`text-base sm:text-lg lg:text-xl font-light capitalize ${
                index === 4 && 'ml-4 sm:ml-8'
              }`}
            >
              {item}
            </a>
